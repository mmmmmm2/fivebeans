...
FiveBeansClient.prototype.tryHandlingResponse = function()
{
	...
	{
		var newBuffer = handler.process(this.buffer);
		if (handler.complete)
		{
			// shift it off & reset
			this.handlers.shift();
...
ResponseHandler.prototype.process = function(data)
{
	...
	if (eol > -1)
	{
		...
		// body is specified by the last arg everything after.
		...
		if (this.RESPONSES_REQUIRING_BODY[response]) {
			// The size doesn't include the trailing CRLF, so add it on
			var bodySize = parseInt(this.args[this.args.length - 1], 10) + 2;

			if (eol + 2 + bodySize > data.length) {
				// Not enough data to read the body, wait till we get it all
				return data;
			}

			// Remove the CRLF from the body
			this.body = data.slice(eol + 2, eol + 2 + bodySize - 2);
		...
		// If we have any data left, return it to be processed again
		if (data.length > totalRead) {
			return data.slice(totalRead);
		} else {
			// Clear the buffer
			return undefined;
		}
	...
	// Not enough data, keep using this
	return data;
};
...
ResponseHandler.prototype.parseBody = function(how)
{
	...
	{
		case 'yaml':
			this.args.push(yaml.load(this.body));
			break;
